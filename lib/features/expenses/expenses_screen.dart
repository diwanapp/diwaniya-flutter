...
class _ExpensesScreenState extends State<ExpensesScreen>
  ...
  double get _totalOwed => _optimized
      .where((d) => d.to == UserService.currentName)
      .fold(0.0, (s, d) => s + d.amount);
  double get _totalIOwe => _optimized
      .where((d) => d.from == UserService.currentName)
      .fold(0.0, (s, d) => s + d.amount);
  double get _netPosition => _totalOwed - _totalIOwe;
  double get _openSettlementTotal =>
      _optimized.fold(0.0, (s, d) => s + d.amount);
  ...
  double get _totalSinceStart =>
      _activeExpenses.fold(0.0, (s, e) => s + e.amount);

  List<Expense> get _filteredExpenses {
    var list = _expenses.where((e) => e.cancelledBy == null).toList();
    if (_filterCat != null)
      list = list.where((e) => e.category == _filterCat).toList();
    if (_filterPayer != null)
      list = list.where((e) => e.payer == _filterPayer).toList();
    return list;
  }
  ...
  void _openEditSheet(Expense expense) {
    ...
    showModalBottomSheet(
        ...
        builder: (_) => _AddExpenseSheet(
              ...
              categories: _cats,
              onSave: (updated) {
                _editExpense(expense.id, updated);
              },
              onCategoryAdded: (cat) {
  ...
  Future<void> _confirmSettlement(String settlementId) async {
    try {
      await ExpenseService.confirmSettlement(settlementId,
          diwaniyaId: _activeDiwaniyaId);
      if (!mounted) return;
      setState(() {});
    } catch (e) {
  ...
  void _openFilter() {
    showModalBottomSheet(
        ...
        builder: (_) => _FilterSheet(
              members: _members,
              categories: _cats,
              currentCat: _filterCat,
              currentPayer: _filterPayer,
              onApply: (cat, payer) => setState(() {
                _filterCat = cat;
                _filterPayer = payer;
              }),
              onClear: () => setState(() {
  ...
  void _openAnalytics() {
    showModalBottomSheet(
        ...
        builder: (_) => _AnalyticsSheet(
            expenses: _activeExpenses, settlements: _settlements));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Padding(
            padding: const EdgeInsets.symmetric(horizontal: 20),
            child: _TabBarW(controller: _tab)),
...
class _AddExpenseSheetState extends State<_AddExpenseSheet> {
    ...
    switch (_splitType) {
      case 'percentage':
        ...
        for (int i = 0; i < widget.members.length; i++) {
          if (!_participants.any((m) => m.name == widget.members[i].name))
            continue;
  ...
  Map<String, double> _computeShares(double amount) {
    ...
    switch (_splitType) {
      case 'percentage':
        for (int i = 0; i < widget.members.length; i++) {
          if (!participants.any((m) => m.name == widget.members[i].name))
            continue;
          final pct = double.tryParse(_pctCtrls[i].text) ?? 0;
          if (pct > 0) {
            shares[widget.members[i].name] = amount * pct / 100;
          }
        }
        break;
      case 'fixed':
        for (int i = 0; i < widget.members.length; i++) {
          ...
          if (fix > 0) {
            shares[widget.members[i].name] = fix;
          }
        }
        break;
      default:
  ...
  void _save() {
    if (!_isValid) return;
    final title =
        _titleCtrl.text.trim().isEmpty ? _selCat! : _titleCtrl.text.trim();
    if (_isEditMode) {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        ...
        padding: EdgeInsets.fromLTRB(
            20, 14, 20, MediaQuery.of(context).viewInsets.bottom + 18),
        decoration: BoxDecoration(
            color: c.card,
            borderRadius:
        ...
        child: SingleChildScrollView(
            child: Column(
              ...
              const SizedBox(height: 16),
              Center(
                  child: Text(_isEditMode ? Ar.editExpense : Ar.addExpense,
                      style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.w700,
                          color: c.t1))),
              ...
              Container(
                ...
                child: TextField(
                  ...
                  style: TextStyle(
                      fontSize: 32, fontWeight: FontWeight.w900, color: c.t1),
                  ...
                  SizedBox(
                      ...
                      child: ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            final sel = _payer == m.name;
                            return GestureDetector(
                                onTap: () => setState(() => _payer = m.name),
                                child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                        color: sel
                                            ? c.accentMuted
                                            : c.inputBg.withValues(alpha: 0.72),
                          ...
                          child: Container(
                              ...
                              decoration: BoxDecoration(
                                  color: sel
                                      ...
                                      : c.inputBg.withValues(alpha: 0.74),
                                  borderRadius: BorderRadius.circular(14),
                                  border: Border.all(
                                      color: sel
                                          ? cat.color.withValues(alpha: 0.36)
                                          : c.divider.withValues(alpha: 0.08))),
                              child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(cat.icon,
                                        size: 14,
                                        color: sel ? cat.color : c.t3),
                                    ...
                                    Text(cat.name,
                                        style: TextStyle(
                                            ...
                                            fontWeight: sel
                                                ? FontWeight.w600
                                                : FontWeight.w500,
                                            color: sel ? cat.color : c.t2))
                                  ])));
                    }),
                    if (UserService.isManager(
                        widget.members.isNotEmpty ? currentDiwaniyaId : null))
                      GestureDetector(
                          onTap: () {
                                  ...
                                  return AlertDialog(
                                      ...
                                      content: TextField(
                                          ...
                                          decoration: InputDecoration(
                                              ...
                                              hintStyle:
                                                  TextStyle(color: dc.t3))),
                                      actions: [
                                        TextButton(
                                            onPressed: () => Navigator.pop(d),
                                            child: Text(Ar.cancel,
                                                style:
                                                    TextStyle(color: dc.t2))),
                                        TextButton(
                                            onPressed: () {
                                              final name = nc.text.trim();
                                              if (name.isEmpty) return;
                                              final exists =
                                                  _localCategories.any((cat) =>
                                                      cat.name.trim() == name);
                ...
                children: [
                  ...
                  if (_useSelectedMembers) ...[
                    ...
                    Wrap(
                        ...
                        children: widget.members.map((m) {
                          final on = _selected.contains(m.name);
                          return GestureDetector(
                              onTap: () => setState(() {
                                    on
                  ...
                  if (_splitType == 'percentage')
                    ...List.generate(widget.members.length, (i) {
                      if (!_participants
                          .any((m) => m.name == widget.members[i].name))
                      ...
                      return Padding(
                          ...
                          child: Row(children: [
                            SizedBox(
                                width: 60,
                                child: Text(widget.members[i].name,
                                    style:
                                        TextStyle(fontSize: 13, color: c.t1))),
                            Expanded(
                                child: SizedBox(
                                    height: 38,
                                    child: TextField(
                                        controller: _pctCtrls[i],
                                        keyboardType: TextInputType.number,
                                        textAlign: TextAlign.center,
                                        style: TextStyle(
                        ...
                        Expanded(
                            child: Text(
                                ...
                                style: TextStyle(
                                    fontSize: 13,
                                    color:
                                        _receiptPath != null ? c.accent : c.t2,
...
Widget _splitChip(String label, bool sel, VoidCallback onTap, CL c) =>
    GestureDetector(
        onTap: onTap,
        child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
                color: sel ? c.accent.withValues(alpha: 0.15) : c.inputBg,
                borderRadius: BorderRadius.circular(10),
                border: sel
...
class _FilterSheet extends StatefulWidget {
  final List<DiwaniyaMember> members;
  final List<ExpenseCategory> categories;
  final String? currentCat, currentPayer;
...
class _FilterSheetState extends State<_FilterSheet> {
  ...
  void initState() {
    ...
    _cat =
        hidden.contains(widget.currentCat?.trim()) ? null : widget.currentCat;
    ...
    return Container(
        ...
        child: Column(
            ...
            children: [
              ...
              Text('التصنيف',
                  style: TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w600, color: c.t2)),
              const SizedBox(height: 8),
              Wrap(spacing: 8, runSpacing: 8, children: [
                _filterChip(Ar.allCategories, _cat == null,
                    () => setState(() => _cat = null), c),
                ...widget.categories
                    .where((cat) => !{
                          'لوازم الكيف',
                          'راتب العامل',
                          'راتب عامل',
                          'أخرى'
                        }.contains(cat.name.trim()))
                    .map((cat) => _filterChip(cat.name, _cat == cat.name,
                        () => setState(() => _cat = cat.name), c)),
              ]),
              const SizedBox(height: 16),
              Text('الشخص',
                  style: TextStyle(
              ...
              const SizedBox(height: 8),
              Wrap(spacing: 8, runSpacing: 8, children: [
                _filterChip(Ar.allMembers, _payer == null,
                    () => setState(() => _payer = null), c),
                ...widget.members.take(6).map((m) => _filterChip(
                    m.name,
                    _payer == m.name,
                    () => setState(() => _payer = m.name),
                    c)),
              ]),
                    ...
                    child: ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                            ...
                            shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(12)),
                            padding: const EdgeInsets.symmetric(vertical: 14)),
                        child: Text(Ar.applyFilter,
                            style: TextStyle(color: c.tInverse)))),
              ]),
            ]));
  }
}
...
Widget _filterChip(
        String label, bool sel, VoidCallback onTap, CL c) =>
    GestureDetector(
        onTap: onTap,
        child: Container(
...
class _AnalyticsSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final confirmedSettled =
        settlements.where((s) => s.confirmed).fold(0.0, (s, e) => s + e.amount);

    final byCat = <String, double>{};
    for (final e in expenses) {
      byCat[e.category] = (byCat[e.category] ?? 0) + e.amount;
    }
    final catE = byCat.entries.toList()
    ...
    final payE = byPayer.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Center(
                child: Text('قراءة شهرية وسنوية لمصاريف الديوانية',
              ...
              child: Column(
                  children: payE
                      ...
                      .map((e) => _Bar(
                          label: e.key,
                          value: e.value,
                          maxVal: payE.isEmpty ? total : payE.first.value,
                          c: c,
                          color: c.info))
                      .toList()),
            ),
              ...
              child: Column(
                  children: catE
                      ...
                      .map((e) => _Bar(
                          label: e.key,
                          value: e.value,
                          maxVal: catE.isEmpty ? total : catE.first.value,
                          c: c,
                          color: c.accent))
                      .toList()),
            ),
...
class _AnalyticsKpi extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 10),
          Text('${value.toInt()} ر.س',
              style: TextStyle(
                  fontSize: 18, fontWeight: FontWeight.w900, color: c.t1)),
          const SizedBox(height: 2),
...
class _TrendKpi extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final text = up
        ...
        : (down ? '-${delta.abs().toStringAsFixed(0)}%' : 'مستقر');
    return Container(
      padding: const EdgeInsets.all(13),
      decoration: BoxDecoration(
          color: c.inputBg.withValues(alpha: 0.62),
    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: points.map((point) {
          ...
          final active = point.month.year == DateTime.now().year &&
              point.month.month == DateTime.now().month;
          return Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(point.value > 0 ? point.value.toInt().toString() : '',
                      style: TextStyle(
                          fontSize: 8,
                          fontWeight: FontWeight.w700,
                  ...
                  AnimatedContainer(
                    ...
                    height: height,
                    decoration: BoxDecoration(
                      color: (active ? c.accent : c.info)
                          .withValues(alpha: active ? 0.86 : 0.46),
                      borderRadius: const BorderRadius.vertical(
                          top: Radius.circular(8), bottom: Radius.circular(3)),
                    ),
                  ),
                  const SizedBox(height: 7),
                  Text(_monthLabel(point.month),
                      style: TextStyle(
                          fontSize: 9,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Flexible(
              child: Text(label,
                  style: TextStyle(
                      fontSize: 12, color: c.t2, fontWeight: FontWeight.w600),
                  overflow: TextOverflow.ellipsis)),
          const SizedBox(width: 8),
          Text('${value.toInt()} ر.س',
              style: TextStyle(
        ...
        ClipRRect(
            ...
            child: SizedBox(
                ...
                child: LinearProgressIndicator(
                    value: maxVal > 0 ? min(1, value / maxVal) : 0,
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
        ...
        child: Container(
            ...
            height: 38,
            decoration: BoxDecoration(
                color: c.card, borderRadius: BorderRadius.circular(11)),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(18),
        border: Border(
            right: BorderSide(color: color.withValues(alpha: 0.55), width: 4)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
          ...
          Text(amountText,
              style: TextStyle(
                  fontSize: isPositive || isNegative ? 22 : 13,
                  fontWeight: FontWeight.w900,
                  color: isPositive || isNegative ? color : c.t2)),
        ]),
        const SizedBox(height: 14),
        Row(children: [
          Expanded(
              child: _MiniMoney(
                  label: 'لك', amount: owedToMe, color: c.success, c: c)),
          const SizedBox(width: 8),
          Expanded(
              child: _MiniMoney(
                  label: 'عليك', amount: iOwe, color: c.error, c: c)),
          const SizedBox(width: 8),
          Expanded(
              child: _MiniMoney(
                  label: 'تسويات مفتوحة',
...
class _MiniMoney extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label,
              style: TextStyle(
                  fontSize: 10, color: c.t3, fontWeight: FontWeight.w600),
              maxLines: 1,
              overflow: TextOverflow.ellipsis),
          const SizedBox(height: 3),
...
class _BalCard extends StatelessWidget {
  ...
  final bool positive;
  const _BalCard(
      {required this.label, required this.amount, required this.positive});
  @override
  Widget build(BuildContext context) {
    final c = context.cl;
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border(
                right:
                    BorderSide(color: col.withValues(alpha: 0.45), width: 3))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(
                width: 26,
                height: 26,
                decoration: BoxDecoration(
                    color: (positive ? c.successM : c.errorM),
                ...
                child: Icon(
                    ...
                    color: col)),
            const SizedBox(width: 8),
            Text(label, style: TextStyle(fontSize: 13, color: c.t2)),
          ]),
          const SizedBox(height: 10),
          Text('${amount.toInt()} ر.س',
              style: TextStyle(
                  fontSize: 22, fontWeight: FontWeight.w700, color: col)),
        ]));
  }
}
...
class _MonthBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final c = context.cl;
    final paidPct =
        total > 0 ? (settled / total * 100).clamp(0, 100).round() : 0;
    return Container(
    ...
    return Container(
        ...
        child: TabBar(
            ...
            indicator: BoxDecoration(
                color: c.cardElevated, borderRadius: BorderRadius.circular(8)),
...
class _TxTab extends StatefulWidget {
  ...
  const _TxTab(
      {required this.expenses, required this.onDelete, required this.onEdit});
  @override
...
class _TxTabState extends State<_TxTab> with AutomaticKeepAliveClientMixin {
    ...
    showModalBottomSheet(
        ...
        builder: (_) => Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 4),
              Text(
                  '${e.amount.toInt()} ر.س · ${e.payer} · ${_timeAgo(e.createdAt)}',
                  style: TextStyle(fontSize: 12, color: c.t3)),
              const SizedBox(height: 4),
              Text('أضافه ${e.createdBy} · ${_timeAgo(e.createdAt)}',
                  style: TextStyle(fontSize: 11, color: c.t3)),
              if (e.updatedBy != null)
                Text('عدّله ${e.updatedBy} · ${_timeAgo(e.updatedAt!)}',
                    style: TextStyle(fontSize: 11, color: c.t3)),
              if (e.hasReceipt) ...[
                const SizedBox(height: 10),
                ClipRRect(
                    borderRadius: BorderRadius.circular(8),
                    child: Image.file(File(e.receiptPath!),
                        height: 100,
                        width: double.infinity,
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Container(
                ...
                SizedBox(
                    ...
                    child: ElevatedButton.icon(
                        ...
                        icon: Icon(Icons.edit_rounded,
                            size: 18, color: c.tInverse),
                        label: Text(Ar.editExpense,
                            style: TextStyle(color: c.tInverse, fontSize: 14)),
                        style: ElevatedButton.styleFrom(
                            backgroundColor: c.accent,
                            elevation: 0,
                            shape: RoundedRectangleBorder(
                              ...
                              builder: (d) {
                                ...
                                return AlertDialog(
                                    ...
                                    shape: RoundedRectangleBorder(
                                        borderRadius:
                                            BorderRadius.circular(16)),
                                    title: Text(Ar.deleteExpense,
                                        style: TextStyle(
                                            fontSize: 16,
                                            fontWeight: FontWeight.w600,
                                            color: dc.t1)),
                                    content: Text(Ar.deleteConfirm,
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.expenses.isEmpty)
      return Center(
          child: Text('لا توجد مصاريف', style: TextStyle(color: c.t3)));
    return ListView.separated(
      ...
      itemBuilder: (_, i) {
        ...
        final catIcon = defaultExpenseCategories
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                        ...
                        const SizedBox(height: 3),
                        Text(
                            '${e.payer} دفع · ${_timeAgo(e.createdAt)} · ${e.category}',
                            style: TextStyle(fontSize: 11, color: c.t3)),
                      ])),
    ...
    return ListView(
        ...
        children: [
          ...
          Container(
              ...
              child: Row(children: [
                Icon(Icons.auto_fix_high_rounded, size: 16, color: c.accent),
                const SizedBox(width: 8),
                Expanded(
                    child: Text(Ar.optimizedSettlements,
                        style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w600,
                            color: c.accent))),
                Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(6)),
                    child: Text('${widget.debts.length} تحويلات',
                        style: TextStyle(
                            fontSize: 10,
                            fontWeight: FontWeight.w600,
                            color: c.accent))),
          ...
          ...widget.debts.map((d) {
            final fromM =
                widget.members.where((m) => m.name == d.from).firstOrNull;
            return Padding(
                padding: const EdgeInsets.only(bottom: 6),
                child: Container(
                    padding: const EdgeInsets.symmetric(
                    ...
                    child: Row(children: [
                      CircleAvatar(
                          radius: 16,
                          backgroundColor: (fromM?.avatarColor ?? c.accent)
                      ...
                      Builder(builder: (_) {
                        final key =
                            '${d.from}->${d.to}:${d.amount.toStringAsFixed(2)}';
                        final isBusy = _settling.contains(key);
                        final canSettle = !isBusy &&
                            (widget.currentUser == d.from ||
                                widget.currentUser == d.to);
                        return GestureDetector(
                          onTap: canSettle
                              ? () async {
                                  ...
                                  try {
                                    await widget.onSettle(
                                        d.from, d.to, d.amount);
                                  } finally {
                                    if (mounted)
                                      setState(() => _settling.remove(key));
                                  }
                                }
                              : null,
                          child: Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                        Text('${s.from} دفع لـ ${s.to}',
                            style: TextStyle(
                                fontSize: 13,
                                fontWeight: FontWeight.w600,
                                color: c.t1)),
                        const SizedBox(height: 2),
                        Text(_timeAgo(s.date),
                            style: TextStyle(fontSize: 11, color: c.t3)),
                      ])),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text('${s.amount.toInt()} ر.س',
                        style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w700,
                            color: c.success)),
                    ...
                    Builder(builder: (_) {
                      ...
                      final canConfirm =
                          !isBusy && widget.currentUser == s.to && !s.confirmed;
                      return GestureDetector(
                        onTap: canConfirm
                            ? () async {
                                ...
                                try {
                                  ...
                                  if (mounted)
                                    setState(() => _confirming.remove(s.id));
                                }
                              }
                            : null,
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: s.confirmed
                                ? c.successM
                                : (canConfirm ? c.accentSurface : c.inputBg),
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(
                              color: s.confirmed
